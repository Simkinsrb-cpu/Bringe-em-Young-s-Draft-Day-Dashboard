[index.html](https://github.com/user-attachments/files/30797705/index.html)
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .wrap{max-width:1180px;margin:0 auto;position:relative;padding-top:6px;}
  ...
  .ticker-item .tpos{color:var(--teal); font-weight:700;}
  .ticker-item .tname{color:var(--text); font-weight:600; text-transform:none;}
  .ticker-item .trank{
  ...
  .modal-card{
    background:linear-gradient(160deg,#233250,#1a2740 55%,#152034);
    border:1px solid rgba(45,212,191,.4);
    border-radius:var(--radius-lg);
    padding:26px;
    width:100%;
    max-width:340px;
    box-shadow:0 30px 60px -14px rgba(0,0,0,.6), 0 0 46px -12px rgba(45,212,191,.45);
    position:relative;
    overflow:hidden;
  }
  .modal-card::before{
  ...
  .team-badge-text .owner-name{
    font-family:'Inter',sans-serif;
    font-size:14px; font-weight:600; color:var(--text);
    margin-top:6px;
  }
  .team-badge-text .owner-title{
    font-family:'JetBrains Mono',monospace;
  ...
  <header>
    <div class="team-badge">
      ...
      <div class="team-badge-text">
        ...
        <div class="owner-name">Robert Simkins</div>
        <div class="owner-title">President &amp; General Manager</div>
      </div>
    </div>
    <div class="hdr-title">
...
function buildTicker() {
  ...
  const itemsHtml = POSITION_LEADERS.map(p =>
    `<span class="ticker-item"><span class="tpos">${p.pos}1</span><span class="tname">${p.name}</span><span class="trank">(${p.rank})</span></span>`
  ).join('<span class="ticker-item" aria-hidden="true">&#8226;</span>');
  // duplicated once back-to-back so the marquee loop (0 to -50%) is seamless
